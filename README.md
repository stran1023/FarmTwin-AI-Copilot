...
> Screenshots below are real captures against a live backend + live Snowflake account (2026-08-06), not mockups.
...
<details>
<summary><strong>Asset Detail — live readings, Harvest Planner, Yield Estimate, all agent-narrated</strong></summary>

![Asset detail](docs/screenshots/asset-detail.png)

Greenhouse A, healthy (90/100): real live sensor readings, a Harvest Planner narrative that honestly reports "readiness isn't trending upward, so no reliable ETA can be projected" rather than fabricating one, and a Yield Estimate ("273 kg, 80% confidence") computed from this asset's real 3-cycle historical average and adjusted for its current health score — every number here is computed, the agent only narrates it.

</details>

<details>
<summary><strong>AI Copilot — free-form, grounded Q&A with real conversational memory</strong></summary>

![Copilot](docs/screenshots/copilot.png)

Asked "What should I do today?" with no asset specified — the agent reasons across all 5 assets in one answer: leads with the good news (all reading normal right now), then surfaces three real recent scares worth following up — Tilapia Pond A's dissolved-oxygen crash (explicitly tied to the same 2024 die-off precedent shown elsewhere in the app) plus a low aerator-stock warning, a resolved feed-hopper dip in the coop, and a resolved fungal-risk episode in the greenhouse — ending on one concrete next step. The trash icon (top right) is the real "Clear conversation" control from feat-063.

</details>

**Walkthrough:** Open the Farm Overview to see the live digital twin at its healthy baseline → click "Run AI Farm Analysis" to watch a real tick and see whether it escalates an asset → open any asset to see Harvest Planner and Yield Estimation working together on real sensor data → visit the AI Copilot and ask a free-form question to see the same farm-wide reasoning in conversational form.
...
Everything else originally on the roadmap (disease prediction, cost optimization, resource planning, water-usage optimization, autonomous daily planning) was triaged and deliberately **not** built — see the Innovation and Challenges sections above for why those calls were made.
